...
export function Approach() {
  return (
    <section className="bg-wytes-cream py-16 md:py-32" data-nav-theme="light">
      <div className="mx-auto max-w-6xl px-4 md:pr-10 md:pl-0">
        <div className="md:flex md:items-start md:gap-8">
          <RevealOnScroll className="md:sticky md:top-32 md:w-1/3 md:flex-none">
            ...
            <h2 className="mt-5 font-display text-3xl uppercase leading-tight text-wytes-ink sm:text-5xl md:mt-8 md:text-5xl">
              Think Deeply.
              <br />
          ...
          <div className="mt-10 border-t border-wytes-ink/10 md:mt-0 md:w-2/3 md:flex-none">
            {STEPS.map((step, index) => (
              <RevealOnScroll key={step.number} delay={index * 0.05} start="top 92%">
                <div className="flex flex-col gap-1 border-b border-wytes-ink/10 py-4 md:flex-row md:items-baseline md:gap-8 md:py-8">
                  <span className="font-mono text-xs uppercase tracking-[0.2em] text-wytes-ink/40 md:w-16 md:flex-none">
                    {step.number} —
                  </span>
                  <h3 className="font-display text-xl uppercase leading-none text-wytes-ink md:w-56 md:flex-none md:text-3xl">
                    {step.title}
                  </h3>
                  <p className="max-w-lg font-body text-xs text-wytes-ink/60 md:text-base">
                    {step.description}
                  </p>
                </div>
              </RevealOnScroll>
